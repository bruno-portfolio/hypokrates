...
### Refactored
- **hypothesis() label handling**: refactored from `check_label()` → `label_events()` directly. Enables access to `indications_text` without extra HTTP calls. Dead code `_format_label_detail()` removed
- **Abstract snippet formatting**: extracted `_format_article_lines()` helper in `_shared.py`, eliminating duplication between `format_references()` and `format_categorized_references()`

- **`full_report()` — complete pharmacovigilance report**: orchestrates `investigate()` + `scan_drug()` + `compare_signals()` in parallel via `asyncio.gather()`. Returns `FullReportResult` with all analyses + `SynthesisDirection` block (signal strength, replication ratio, class effect, demographic bias, mechanism plausibility, label status). New models: `SignalStrength` (StrEnum), `SynthesisDirection`, `FullReportResult`. MCP tool `full_report` formats 11 markdown sections. 15 new tests. MCP tools: 46 → 47
...
### Fixed
- **OnSIDES DuckDB schema**: 6 de 7 tabelas tinham colunas faltando ou na ordem errada vs os CSVs reais — `vocab_rxnorm_ingredient` (faltava `rxnorm_term_type`), `product_label` (+2 cols), `product_adverse_effect` (+3 cols, ordem diferente), `vocab_rxnorm_product`/`vocab_meddra_adverse_effect` (nome do campo `term_type` → `rxnorm_term_type`/`meddra_term_type`), `vocab_rxnorm_ingredient_to_product` (colunas invertidas). Causava "Binder Error: table has 2 columns but 3 values were supplied" em 100% das queries
- **PharmGKB annotations parser**: API real usa `location.genes[].symbol` (não `relatedGenes`), `levelOfEvidence.term` (não `evidenceLevel` string), `types[]` (não `phenotypeCategories[].term`), e `accessionId` (não `id`). Tabela de clinical annotations renderizava com gene/evidence/categories vazios. Parser agora suporta ambos formatos (real + golden data)
- **Canada Vigilance parser**: arquivos bulk não têm header row — `csv.DictReader` tratava 1a linha de dados como header, resultando em 0 reports carregados. Reescrito com `csv.reader` + acesso posicional por índice de coluna. Também corrigido filename mismatch (`Drug_Product.txt` → `drug_products.txt` em alguns extracts) com `_find_file()` que tenta nomes alternativos. Corrigido `Drug_Product_Ingredients.txt` que tem 5 colunas (não 2) — `DRUG_PRODUCT_ID` na col 1, `ACTIVE_INGREDIENT_NAME` na col 4
