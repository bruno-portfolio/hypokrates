...
from tests.helpers import make_events, make_meta, make_signal
...
async def test_class_effect(
    ...
    async def _signal_side_effect(drug: str, event: str, **kwargs: object) -> SignalResult:
        return make_signal(drug=drug, event=event, prr=2.0, detected=True)
...
async def test_drug_specific(
    ...
    async def _signal_side_effect(drug: str, event: str, **kwargs: object) -> SignalResult:
        if event == "RASH" and drug == "drug_a":
            return make_signal(drug=drug, event=event, prr=5.0, detected=True)
        return make_signal(drug=drug, event=event, prr=0.8, detected=False)
...
async def test_differential_by_count(
    ...
    async def _signal_side_effect(drug: str, event: str, **kwargs: object) -> SignalResult:
        if drug in ("drug_a", "drug_b"):
            return make_signal(drug=drug, event=event, prr=3.0, detected=True)
        return make_signal(drug=drug, event=event, prr=0.5, detected=False)
...
async def test_differential_by_outlier(
    ...
    async def _signal_side_effect(drug: str, event: str, **kwargs: object) -> SignalResult:
        ...
        return make_signal(drug=drug, event=event, prr=prr_map[drug], detected=True)
...
async def test_empty_events(mock_top: AsyncMock) -> None:
    ...
    mock_top.side_effect = [
        FAERSResult(events=[], meta=make_meta()),
        FAERSResult(events=[], meta=make_meta()),
    ]
...
async def test_operational_filtered(
    ...
    mock_top.side_effect = [
        make_events(["OFF LABEL USE", "RASH"]),
        make_events(["DRUG INEFFECTIVE", "RASH"]),
    ]
...
async def test_meddra_dedup(
    ...
    mock_top.side_effect = [
        make_events(["ANAPHYLACTIC REACTION"]),
        make_events(["ANAPHYLACTIC SHOCK"]),
    ]
